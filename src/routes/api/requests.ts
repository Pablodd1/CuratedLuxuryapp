...
app.get('/', async (c) => {
  try {
    ...
    const count = await db.prepare('SELECT COUNT(*) as total FROM client_requests').first<{ total: number }>()
...
app.post('/', async (c) => {
  try {
    ...
    await c.env.DB.prepare(`INSERT INTO client_requests (id, owner_id, client_name, looking_for_brand, looking_for_model, reference_number, budget_usd, currency, urgency, condition_required, notes, status, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(id,
        body.owner_id || 'demo-user',
...
app.put('/:id', async (c) => {
  try {
    ...
    await c.env.DB.prepare(`UPDATE client_requests SET client_name=?, looking_for_brand=?, looking_for_model=?, reference_number=?, budget_usd=?, currency=?, urgency=?, condition_required=?, notes=?, status=?, updated_at=? WHERE id=?`)
...
app.delete('/:id', async (c) => {
  try {
    const id = c.req.param('id')
    const now = new Date().toISOString()
    await c.env.DB.prepare('UPDATE client_requests SET status=?, updated_at=? WHERE id=?').bind('cancelled', now, id).run()
...
app.get('/stats/summary', async (c) => {
  try {
    const db = c.env.DB
    const total = await db.prepare('SELECT COUNT(*) as c FROM client_requests WHERE status != ?').bind('cancelled').first<{ c: number }>()
    ...
    const totalBudget = await db.prepare('SELECT SUM(budget_usd) as v FROM client_requests WHERE status = ?').bind('active').first<{ v: number }>()
